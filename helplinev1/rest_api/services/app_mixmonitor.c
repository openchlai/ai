...
#define AII_SERVER_IP "127.0.0.1"
#define AII_SERVER_PORT 8300

static int aii_sock_open (void)
{
	int f;
	...
	if ( (f = socket (AF_INET, SOCK_STREAM, 0))==-1 ) // create
	{
		ast_log(LOG_ERROR, "\x1b[31m[error] socket create failed! \x1b[0m");
		perror ("\x1b[31m[error] socket create failed! \x1b[0m");
		return -1;
	}

	int on = 1;
	if ( setsockopt (f, SOL_SOCKET, SO_REUSEADDR, (const char*) &on, sizeof (on)) == -1)
	{
		ast_log(LOG_ERROR, "\x1b[31m[error] setsockopt(REUSEADDR) failed! \x1b[0m");
		perror ("\x1b[31m[error] setsockopt(REUSEADDR) failed! \x1b[0m");
		return -1;
	}

	addr.sin_family = AF_INET;
	addr.sin_port = htons (AII_SERVER_PORT);

	if (inet_pton (AF_INET, AII_SERVER_IP, &(addr.sin_addr)) < 1)
	{
		ast_log(LOG_ERROR, "\x1b[31m[error] address not supported. inet_pton failed! \x1b[0m| 127.0.0.1\n");
		return -1;
    	}

	if (errno == EAFNOSUPPORT)
	{
		perror ("\x1b[31m[error] connection not supported! \x1b[0m");
		ast_log (LOG_ERROR, "connection not supported fd:%d\n", f);
		return -1;
	}

	if (connect (f, (struct sockaddr*) &(addr), sizeof (addr)) != 0)
	{
		perror ("\x1b[31m[error] connect() failed! \x1b[0m");
		ast_log (LOG_ERROR, "connect failed fd:%d\n", f);
		return -1;
	}
...
static int aii_sock_write (struct ast_frame * f, int fd)
{
        int res;

        if (!f->datalen)
        {
		ast_log(LOG_ERROR, "Invalid datalen:%d\n", f->datalen);
                return -1;
        }

        if ((res = write(fd, f->data.ptr, f->datalen)) != f->datalen )
	{
 		ast_log(LOG_ERROR, "Bad write (%d:%s) | %d\n", res, strerror(errno), fd);
                return -1;
        }
...
static void *mixmonitor_thread(void *obj)
{
	...
	struct ast_trans_pvt* trans_pvt = NULL;
	struct ast_format* fmtsrc = NULL;
	struct ast_format* fmtdst = NULL;
	unsigned int srsrc = 0;
        unsigned int srdst = 16000;
	int ll = 0;
	int aii_fd = aii_sock_open ();
	if (aii_fd<3)
	{
		ast_verb(2, "AII Connection Failed -----------------------\n");
	}
	...
	while (mixmonitor->audiohook.status == AST_AUDIOHOOK_STATUS_RUNNING && !mixmonitor->mixmonitor_ds->fs_quit) {
		...
		struct ast_frame *fr_read_ = NULL;
		...
		unsigned int sr = ast_format_get_sample_rate(fr_read->subclass.format);

		if (trans_pvt==NULL || sr!=srsrc)
		{
 			fmtsrc = ast_format_cache_get_slin_by_rate (sr);
 			fmtdst = ast_format_cache_get_slin_by_rate (srdst);
			trans_pvt = ast_translator_build_path(fmtdst, fmtsrc);
			if (trans_pvt==NULL)
			{
				ast_log(LOG_ERROR, "failed to build translator path %d -> %d %p\n", sr, srdst, trans_pvt);
			}
			else { srsrc=sr; }
		}
		...
		{
			// fr_read_ = fr_read;
			fr_read_ = ast_translate(trans_pvt, fr_read, 0);
			// ast_translator_free_path (trans_pvt);
		}

		if (ll%100==0)
		{
			unsigned int sm = ast_format_get_sample_rate(fr->subclass.format);
			//unsigned int sr = ast_format_get_sample_rate(fr_read->subclass.format);
			unsigned int sw = ast_format_get_sample_rate(fr_write->subclass.format);
			unsigned int sr_ = ast_format_get_sample_rate(fr_read_->subclass.format);
			ast_verb(2, "fr %d %u %u %u %u , %p(%s,%s)\n", ll, sm, sr, sw, sr_, trans_pvt, ast_format_get_name(fmtsrc), ast_format_get_name(fmtdst));
		}
		ll++;
		...
		if (!ast_test_flag(mixmonitor, MUXFLAG_BRIDGED)
			...
			if (aii_fd>2 && fr_read_) // && fr_read_->frametype == AST_FRAME_VOICE)
			{
				struct ast_frame *cur;
				for (cur = fr_read_; cur; cur = AST_LIST_NEXT(cur,frame_list))
				{
					aii_sock_write (cur, aii_fd);
				}
			}
