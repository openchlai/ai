...
function autoexten ()
{
	$q = "select id, exten from auth where exten not like '000' order by exten";
	...
	while ($row = mysqli_fetch_row ($res))
	{
		...
		$p = [];
		$o["user_id"] = $row[0];
		$rt = rest_uri_post ("users", "", $o["user_id"], $o, $p); 
		if ($rt!=202) { echo "ERROR ".$rt."  user_id:".$o["user_id"]; continue; }
	}
