...
def get_gpu_info():
    ...
    for i, gpu in enumerate(x):
        # print(f"GPU {i + 1} Information:")
        gpus["index"] = gpu.id
        gpus["name"] = gpu.name
        gpus["driver"] = gpu.driver
        gpus["totalmem"] = gpu.memoryTotal
        gpus["freemem"] = gpu.memoryFree
        gpus["usedmem"] = gpu.memoryUsed
        gpus["pctload"] = gpu.load * 100
        gpus["tempcels"] = gpu.temperature
