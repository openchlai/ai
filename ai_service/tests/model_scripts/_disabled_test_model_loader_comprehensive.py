...
@pytest.fixture
def mock_settings():
    """Create a real Settings instance with test values"""
    ...
    settings = Settings(
        ...
        hf_translator_model=None,
        hf_ner_model=None,
        hf_classifier_model=None,
        hf_summarizer_model=None,
        hf_qa_model=None
    )
...
class TestModelLoaderInitialization:
    ...
    def test_model_loader_init_with_disabled_loading(self, mock_settings):
        ...
        # Create a new settings instance with model loading disabled
        from app.config.settings import Settings
        import tempfile
...
class TestLoadAllModels:
    ...
    @pytest.mark.asyncio
    async def test_load_all_models_success(self, mock_qa, mock_sum, mock_class,
                                             mock_ner, mock_trans, mock_whisper, mock_settings):
...
class TestModelStatus:
    ...
    def test_is_model_ready(self, mock_settings, mock_all_models):
        ...
        from app.model_scripts.model_loader import ModelLoader, ModelStatus

        mock_settings.initialize_paths = MagicMock()
        mock_settings.models_path = "/models"

        loader = ModelLoader()

        # Create model status for whisper
        whisper_status = ModelStatus("whisper")
        whisper_status.loaded = True
        whisper_status.error = None
        loader.model_status["whisper"] = whisper_status
