...
class TestAsteriskTCPServer:
    ...
    async def test_submit_transcription(self, server):
        ...
        with patch('app.tasks.audio_tasks.process_streaming_audio_task.delay') as mock_task:
            mock_task.return_value.id = "test_task_456"
            
            await server._submit_transcription(audio_array, connection_id)
            
            # Verify task was submitted
            mock_task.assert_called_once()
            call_args = mock_task.call_args[1]  # keyword arguments
            
            assert call_args["connection_id"] == connection_id
            assert call_args["language"] == "sw"
    ...
    async def test_audio_chunk_processing(self):
        ...
        with patch('app.streaming.audio_buffer.AsteriskAudioBuffer') as mock_buffer_class:
            ...
            with patch.object(server, '_submit_transcription') as mock_submit:
                ...
                mock_submit.assert_called_once_with(mock_audio_array, 'test_connection')
