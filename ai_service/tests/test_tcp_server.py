...
class TestAsteriskTCPServer:
    ...
    @pytest.mark.asyncio
    async def test_submit_transcription_success(self, tcp_server):
    ...
    @pytest.mark.asyncio
    async def test_submit_transcription_failure(self, tcp_server):
    ...
    async def test_start_server_success(self, tcp_server):
        ...
        with patch('asyncio.start_server', return_value=mock_server) as mock_start:
            ...
            server_task = asyncio.create_task(tcp_server.start_server("127.0.0.1", 8300))
        ...
        mock_start.assert_called_once_with(tcp_server.handle_connection, "127.0.0.1", 8300)
