...
def mock_agent_service():
    ...
    with patch('app.api.call_session_routes.enhanced_notification_service') as mock:
        mock.send_notification = AsyncMock(return_value=True)
        mock.endpoint_url = "http://test-endpoint"
        mock.auth_endpoint_url = "http://test-auth"
        mock.use_base64 = False
        mock.site_id = "test-site"
        mock.bearer_token = "test-token"
        mock.token_expires_at = datetime(2023, 1, 1, 12, 0, 0)
        yield mock
...
class TestAgentIntegrationEndpoints:
    ...
    async def test_agent_notification_health(self, client, mock_agent_service):
        ...
        # Endpoint doesn't use get_health_status, it uses properties directly
        ...
        assert data["endpoint"] == "http://test-endpoint"
        assert data["token_status"]["has_token"] is True
    ...
    async def test_agent_notification_service_unavailable(self, client):
        with patch('app.api.call_session_routes.enhanced_notification_service') as mock_service:
            # Mock it so that accessing endpoint_url raises an exception
            type(mock_service).endpoint_url = PropertyMock(side_effect=Exception("Service unavailable"))
            response = client.get("/api/v1/calls/agent-service/health")
