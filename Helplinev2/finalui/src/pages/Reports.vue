    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex border-b mb-6 dark:border-gray-800">
        ...
        <button @click="activeTab = 'custom'"
          class="px-6 py-3 font-medium text-sm border-b-2 transition-colors flex items-center gap-2" :class="activeTab === 'custom'
            ? (isDarkMode ? 'border-amber-500 text-amber-500' : 'border-amber-600 text-amber-700')
            : 'border-transparent text-gray-500 hover:text-gray-700'">
          Custom Explorer
        </button>
      ...
      <!-- Tab Content: Custom Explorer -->
      <div v-if="activeTab === 'custom'">
        <!-- Controls Section -->
        <div v-if="!isDashboardMode" class="rounded-lg shadow-xl p-6 mb-6 border" :class="isDarkMode
          ...
          <div class="mt-6">
            ...
            <!-- Available Options -->
            <div class="rounded-lg p-4 mb-4 border" :class="isDarkMode
            ...
            <!-- Selected Filters -->
            <div v-if="selectedYAxis.length > 0" class="rounded-lg p-4 border" :class="isDarkMode
              ...
              <div class="flex items-center justify-between mb-2">
                ...
                <button @click="clearAllFilters" class="text-xs font-medium flex items-center gap-1" :class="isDarkMode
              ...
              <div class="flex flex-wrap gap-2">
                <div v-for="(field, index) in selectedYAxis" :key="field"
                  ...
                  <button @click="removeYAxis(index)" class="rounded p-0.5 transition-colors" :class="isDarkMode
                    ? 'hover:bg-amber-700'
                    : 'hover:bg-amber-800'">
        ...
        <!-- Website Statistics Dashboard Mode -->
        ...
        <div v-if="!isDashboardMode" class="rounded-lg shadow-xl p-6 mb-6 border" :class="isDarkMode
          ...
          <div v-if="loading" class="flex items-center justify-center h-96">
            <div class="flex flex-col items-center gap-4">
              <div class="animate-spin rounded-full h-12 w-12 border-4" :class="isDarkMode
                ? 'border-amber-900/30 border-t-amber-500'
                : 'border-amber-900/30 border-t-amber-600'"></div>
              <div class="font-medium" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
                Loading data...
              </div>
            </div>
          </div>
          ...
          <div v-else-if="chartData.length === 0" class="flex items-center justify-center h-96">
            <div class="text-center">
              <i-mdi-chart-bar class="mx-auto h-16 w-16" :class="isDarkMode ? 'text-gray-600' : 'text-gray-400'" />
              <p class="mt-4 font-medium" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
                No data available
              </p>
          ...
          <!-- Bar Chart -->
          <div v-else class="overflow-x-auto rounded-lg p-4 border" :class="isDarkMode
              ...
              <svg :width="svgWidth" :height="svgHeight">
                ...
                <g v-for="tick in yTicks" :key="'grid-' + tick">
                  <line :x1="margin.left" :x2="svgWidth - margin.right" :y1="yScale(tick)" :y2="yScale(tick)"
                    :stroke="isDarkMode ? '#374151' : '#d1d5db'" stroke-width="1" stroke-dasharray="4" />
                </g>

                <!-- Bars -->
                <g v-for="(bar, index) in chartData" :key="'bar-' + index">
                  <rect :x="margin.left + index * (barWidth + barSpacing)" :y="yScale(bar.value)" :width="barWidth"
                    :height="svgHeight - margin.bottom - yScale(bar.value)"
                    :fill="isDarkMode ? 'url(#barGradientDark)' : 'url(#barGradientLight)'"
                    class="cursor-pointer hover:opacity-80 transition-opacity" rx="4" />
                  <!-- Value labels on bars -->
                  <text :x="margin.left + index * (barWidth + barSpacing) + barWidth / 2" :y="yScale(bar.value) - 8"
                  ...
                  <text :x="margin.left + index * (barWidth + barSpacing) + barWidth / 2"
                    :y="svgHeight - margin.bottom + 20" text-anchor="middle" font-size="11" font-weight="500"
                ...
                <g v-for="tick in yTicks" :key="'label-' + tick">
                  <text :x="margin.left - 8" :y="yScale(tick) + 4" text-anchor="end" font-size="11" font-weight="500"
                    :class="isDarkMode ? 'fill-gray-400' : 'fill-gray-600'">
                    {{ tick }}
                  </text>
                </g>
                ...
                <line :x1="margin.left" :x2="svgWidth - margin.right" :y1="svgHeight - margin.bottom"
                  :y2="svgHeight - margin.bottom" :stroke="isDarkMode ? '#6b7280' : '#9ca3af'" stroke-width="2" />

                <!-- Y-axis line -->
                <line :x1="margin.left" :x2="margin.left" :y1="margin.top" :y2="svgHeight - margin.bottom"
                  :stroke="isDarkMode ? '#6b7280' : '#9ca3af'" stroke-width="2" />
        ...
        <div v-if="!isDashboardMode" class="rounded-lg shadow-xl p-6 border" :class="isDarkMode
          ...
          <div v-else class="overflow-x-auto rounded-lg border"
            ...
            <table class="min-w-full divide-y" :class="isDarkMode ? 'divide-gray-700' : 'divide-gray-200'">
              <thead :class="isDarkMode ? 'bg-black/60' : 'bg-gray-50'">
                <tr>
                  ...
                  <th v-for="period in tableTimePeriods" :key="'period-' + period"
                    class="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider" :class="isDarkMode
                      ? 'text-gray-300 bg-amber-900/20'
                      : 'text-gray-700 bg-amber-50'">
                    {{ period }}
                  </th>
                  <th class="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider" :class="isDarkMode
              ...
              <tbody class="divide-y" :class="isDarkMode
                ? 'bg-black divide-gray-700'
                : 'bg-white divide-gray-200'">
                <tr v-for="(row, rowIdx) in tableData" :key="'row-' + rowIdx" class="transition-colors" :class="isDarkMode
                  ? 'hover:bg-gray-700/30'
                  : 'hover:bg-gray-50'">
                  <!-- Filter value cells -->
                  <td v-for="(filter, filterIdx) in selectedYAxis" :key="'cell-filter-' + filterIdx"
                    class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <span class="inline-flex items-center px-3 py-1 rounded-lg" :class="isDarkMode
                  ...
                  <td v-for="(count, periodIdx) in row.counts" :key="'cell-count-' + periodIdx"
                    class="px-6 py-4 whitespace-nowrap text-sm text-center font-medium"
                    :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
                    {{ count }}
                  </td>
                  <!-- Total cell -->
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-bold" :class="isDarkMode
                ...
                <tr class="font-bold" :class="isDarkMode ? 'bg-black/60' : 'bg-gray-100'">
                  ...
                  <td v-for="(total, idx) in columnTotals" :key="'total-' + idx"
                    class="px-6 py-4 whitespace-nowrap text-sm text-center"
                    :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
                    {{ total }}
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-center" :class="isDarkMode
                    ? 'text-amber-500 bg-amber-900/30'
                    : 'text-amber-700 bg-amber-100'">
                    {{ grandTotal }}
                  </td>
...
<script setup>
  ...
  // Check if current mode is dashboard
  const isDashboardMode = computed(() => {
    return endpointConfig[selectedEndpoint.value]?.mode === 'dashboard'
  })
