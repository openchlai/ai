  ...
  <div class="space-y-6">
    ...
    <div v-else>
      ...
      <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center gap-2" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
          <i-mdi-account-multiple class="w-5 h-5" :class="isDarkMode ? 'text-amber-500' : 'text-amber-700'" />
          <span class="text-sm">
        ...
        <div class="flex gap-3">
          <button @click="view = 'timeline'" :class="getViewButtonClass(view === 'timeline')">
            <i-mdi-timeline-text-outline class="w-5 h-5" />
            Timeline
          </button>

          <button @click="view = 'table'" :class="getViewButtonClass(view === 'table')">
      ...
      <!-- Pagination Controls -->
      <Pagination :paginationInfo="store.paginationInfo" :hasNextPage="store.hasNextPage"
        :hasPrevPage="store.hasPrevPage" :loading="store.loading" :pageSize="selectedPageSize" @prev="goToPrevPage"
    ...
    <Transition name="modal">
      <div v-if="showEditModal"
        class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
        <div class="my-8">
          <UserEditForm :user="userToEdit" @saved="handleEditSaved" @cancel="showEditModal = false" />
        </div>
      </div>
    </Transition>
...
<script setup>
  ...
  import UsersTimeline from '@/components/users/UsersTimeline.vue'
