...
<script setup>
  ...
  onMounted(() => {
    ...
    // Auto-restore SIP connection if it was active
    if (localStorage.getItem('sipEnabled') === 'true') {
      setTimeout(() => {
        sipStore.start()
          .then(() => {
              activeCallStore.initializeQueue()
          })
          .catch(err => console.error('[App] SIP auto-restore failed:', err))
      }, 1000) // Small delay to ensure auth is ready
    } else {
       // If SIP not auto-starting, still try to restore queue status
       activeCallStore.initializeQueue()
    }
  })
