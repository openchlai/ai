<template>
  <div class="attachment-upload">
    ...
    <p v-if="description" class="text-sm mb-4 -mt-1" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
      {{ description }}
    </p>
    ...
    <div class="flex flex-col gap-4">
      ...
      <div :class="[
        ...
        isDragOver
          ? isDarkMode
            ? 'border-[#2DD4BF] bg-[#008080]/10 scale-[1.02]'
        ...
        filesStore.loading
          ...
          : isDarkMode
            ? 'hover:border-[#2DD4BF] hover:bg-[#008080]/5'
            : 'hover:border-[#003366] hover:bg-[#003366]/5'
      ]" @dragover.prevent="handleDragOver" @dragleave.prevent="handleDragLeave" @drop.prevent="handleDrop"
        @click="triggerFileInput">
        <input ref="fileInput" type="file" multiple :accept="acceptedTypes" @change="handleFileSelect" class="hidden"
        ...
        <div class="flex flex-col items-center gap-4">
          <div :class="[
            ...
            !filesStore.loading && (isDarkMode ? 'group-hover:text-[#2DD4BF]' : 'group-hover:text-[#003366]')
          ]">
          ...
          <div class="flex flex-col gap-1">
            <p class="text-base font-medium" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
      ...
      <!-- Uploaded Files List -->
      <div v-if="attachments && attachments.length > 0" class="border rounded-lg p-4" :class="isDarkMode
        ? 'bg-black border-transparent'
        : 'bg-white border-transparent'">
        <h4 class="text-sm font-semibold mb-3" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
          Uploaded Files ({{ attachments.length }})
        </h4>
        <div class="flex flex-col gap-2">
          <div v-for="(file, index) in attachments" :key="index"
            class="flex items-center justify-between p-3 border rounded-md transition-all duration-200" :class="isDarkMode
              ? 'bg-gray-700/50 border-transparent hover:border-[#2DD4BF]'
              : 'bg-gray-50 border-transparent hover:border-[#003366]'">
            <div class="flex items-center gap-3 flex-1 min-w-0">
              <div class="flex-shrink-0" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">
              ...
              <div class="min-w-0 flex-1">
                <div class="text-sm font-medium mb-0.5 truncate whitespace-nowrap overflow-hidden"
            ...
            <div class="flex gap-2 items-center">
              <button type="button"
                ...
                :class="isDarkMode
                  ...
                  : 'border-transparent text-gray-500'" @click="removeFile(index)" title="Remove file"
                :disabled="filesStore.loading">
      ...
      <!-- Upload Progress -->
      <div v-if="uploadProgress > 0 && uploadProgress < 100" class="border rounded-lg p-4" :class="isDarkMode
        ? 'bg-black border-transparent'
        : 'bg-white border-transparent'">
        <div class="w-full h-2 rounded overflow-hidden mb-2" :class="isDarkMode ? 'bg-gray-700' : 'bg-gray-200'">
          <div class="h-full transition-all duration-300" :class="isDarkMode ? 'bg-[#008080]' : 'bg-[#003366]'"
            :style="{ width: uploadProgress + '%' }"></div>
        </div>
        <span class="text-xs text-center block" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
          Uploading attachments... {{ Math.round(uploadProgress) }}%
        </span>
      </div>

      <!-- Upload Status -->
      <div v-if="filesStore.loading" class="border rounded-lg p-4" :class="isDarkMode
        ? 'bg-black border-transparent'
        : 'bg-white border-transparent'">
      ...
      <!-- Error Display -->
      <div v-if="filesStore.error" class="border rounded-lg p-4" :class="isDarkMode
        ? 'bg-red-600/20 border-red-600/50'
        : 'bg-red-50 border-red-300'">
        <div class="flex items-center gap-3 text-sm" :class="isDarkMode ? 'text-red-400' : 'text-red-700'">
