<template>
  <div class="py-2 pb-4">
    <div class="relative flex justify-between items-start gap-6 pt-7">
      <!-- Connecting Line -->
      <div class="absolute top-10 left-0 right-0 h-0.5 -z-10" :class="isDarkMode ? 'bg-gray-700' : 'bg-gray-300'"
        style="top: 2.75rem;">
        <div class="h-full transition-all duration-300" :class="isDarkMode ? 'bg-green-500' : 'bg-green-600'"
          :style="{ width: `${((currentStep - 1) / (totalSteps - 1)) * 100}%` }"></div>
      </div>

      <!-- Steps -->
      <div v-for="step in totalSteps" :key="step" class="flex flex-col items-center gap-1.5 flex-1" :class="{
        ...
        <!-- Step Label -->
        <div class="text-[9px] sm:text-xs font-bold text-center transition-colors max-w-[60px] sm:max-w-[100px]"
  ...
  const getStepCircleClass = (step) => {
    ...
    if (isDarkMode.value) {
      ...
      if (status === 'pending' && !isCurrent) {
        ...
        return 'bg-black border-[#2DD4BF] text-[#2DD4BF]'
      ...
      if (status === 'pending' && !isCurrent) {
        ...
        return 'bg-white border-[#003366] text-[#003366]'
      } else if (status === 'completed') {
