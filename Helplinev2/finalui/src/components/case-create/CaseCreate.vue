<template>
  <div class="space-y-6">
    ...
    <PerpetratorModal v-if="perpetratorModalOpen" :perpetrators="formData.step3.perpetrators"
      :perpetratorForm="perpetratorForm" :currentPerpetratorStep="currentPerpetratorStep"
      :perpetratorModalOpen="perpetratorModalOpen" :loading="perpetratorStore.loading"
      @close-modal="closePerpetratorModal" @remove-perpetrator="removePerpetrator"
      @update-perpetrator-form="updatePerpetratorForm" @prev-perpetrator-step="prevPerpetratorStep"
      @next-perpetrator-step="nextPerpetratorStep" @add-perpetrator="addPerpetrator" />
  </div>
</template>
    ...
    setup() {
      ...
      onMounted(() => {
        if (route.query.phone) {
          formData.step1.searchQuery = route.query.phone
        }
        // Capture telephony metadata if present
      ...
      const submitCase = async () => {
        ...
        try {
          ...
          const baseSourceFields = {
            ...
            src_uid2: formData.metadata.src_uid || srcUid2,
            src_callid: formData.metadata.src_callid || srcCallId,
            src_usr: userId,
