  ...
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div
      ...
      <div class="p-6 space-y-6 flex-1 overflow-y-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <TaxonomySelect label="Reporter’s Relationship with Client" v-model="form.relationship"
            root-key="RELATIONSHIP" placeholder="Select Relationship" />
          <BaseTextarea label="Relationship Comment" v-model="form.relationshipComment" placeholder="Comment..."
            :rows="1" />
        </div>

        <!-- Row 6: Household Info -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <BaseInput label="Number of Adults in Household" type="number" v-model="form.adultsInHousehold"
            placeholder="#" />
          <TaxonomySelect label="Household Type" v-model="form.householdType" root-key="HOUSEHOLD_TYPE"
            placeholder="Select Type" />
          <TaxonomySelect label="Head of Household Occupation" v-model="form.headOccupation" root-key="OCCUPATION"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <TaxonomySelect label="Parent/Guardian Marital Status" v-model="form.parentMaritalStatus"
            root-key="MARITAL_STATUS" placeholder="Select Status" />
        </div>

        <!-- Row 8: Parent ID -->
        <div>
          <BaseInput label="Parent/Guardian Identification Number" v-model="form.parentIdNumber"
            placeholder="ID Number" />
        </div>

        <!-- Row 9: Health, HIV, Marital -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <TaxonomySelect label="Client Health Status" v-model="form.healthStatus" root-key="HEALTH_STATUS"
            placeholder="Select Health" />
          <TaxonomySelect label="Client HIV Status" v-model="form.hivStatus" root-key="HIV_STATUS"
            placeholder="Select HIV Status" />
          <TaxonomySelect label="Client Marital Status" v-model="form.maritalStatus" root-key="MARITAL_STATUS"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-semibold mb-2"
              :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">Attending School?</label>
            <div class="flex gap-4">
              <label class="flex items-center gap-2"><input type="radio" v-model="form.attendingSchool" value="1"
                  class="text-[#003366] dark:text-[#2DD4BF] focus:ring-[#003366] dark:focus:ring-[#2DD4BF]"> Yes</label>
              <label class="flex items-center gap-2"><input type="radio" v-model="form.attendingSchool" value="0"
                  class="text-[#003366] dark:text-[#2DD4BF] focus:ring-[#003366] dark:focus:ring-[#2DD4BF]"> No</label>
              <label class="flex items-center gap-2"><input type="radio" v-model="form.attendingSchool" value="2"
          ...
          <div>
            <label class="block text-sm font-semibold mb-2"
              :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">Disabled?</label>
            <div class="flex gap-4">
              <label class="flex items-center gap-2"><input type="radio" v-model="form.isDisabled" value="1"
                  class="text-[#003366] dark:text-[#2DD4BF] focus:ring-[#003366] dark:focus:ring-[#2DD4BF]"> Yes</label>
              <label class="flex items-center gap-2"><input type="radio" v-model="form.isDisabled" value="0"
                  class="text-[#003366] dark:text-[#2DD4BF] focus:ring-[#003366] dark:focus:ring-[#2DD4BF]"> No</label>
              <label class="flex items-center gap-2"><input type="radio" v-model="form.isDisabled" value="2"
                  class="text-[#003366] dark:text-[#2DD4BF] focus:ring-[#003366] dark:focus:ring-[#2DD4BF]">
                Unknown</label>
            </div>
          </div>
      ...
      <div
        ...
        <button @click="handleUpdate" class="px-6 py-2.5 rounded-lg font-medium text-white transition-colors shadow-sm"
