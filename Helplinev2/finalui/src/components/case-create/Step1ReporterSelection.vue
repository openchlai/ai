  ...
  <div class="min-h-96">
    <div class="flex flex-col gap-5">
      <div>
          ...
          <div class="flex gap-3 items-center mb-4">
            <div
              ...
              <input v-model="searchQuery" type="text" placeholder="Search by name or phone..."
                ...
                :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'" @input="handleSearchInput" />
            </div>
            <button type="button"
              class="h-10 px-4 text-white rounded-lg text-sm font-medium transition-all flex items-center gap-2 whitespace-nowrap"
              :class="isDarkMode
                ? 'bg-amber-600 hover:bg-amber-700'
                : 'bg-amber-700 hover:bg-amber-800'" @click="openCreateReporterForm">
        ...
        <div v-if="isLoading" class="flex items-center gap-3 p-5 text-center"
          ...
          <div class="w-5 h-5 border-2 rounded-full animate-spin" :class="isDarkMode
            ...
            : 'border-transparent border-t-amber-600'"></div>
        ...
        <div class="flex flex-col gap-2 max-h-96 overflow-y-auto"
          ...
          <div class="py-2 text-sm border-b mb-3" :class="isDarkMode
            ? 'text-gray-400 border-transparent'
            : 'text-gray-600 border-transparent'">
            <span>{{ filteredContacts.length }} reporter(s) found</span>
          </div>
          <div v-for="contact in filteredContacts" :key="getContactId(contact)"
            class="flex items-center gap-3 border rounded-lg p-3 cursor-pointer transition-all" :class="isSelected(contact)
              ? isDarkMode
            ...
            <div class="flex-1 min-w-0">
              <div class="flex flex-col gap-2">
                <div class="flex flex-col gap-1">
                  ...
                  <div class="text-sm leading-tight" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
                    {{ getValue(contact, 'phone') || 'No phone' }}
                  </div>
                </div>
                <div class="flex items-center gap-2 mt-1">
                  <div class="flex gap-2 flex-wrap">
                    <span v-if="getValue(contact, 'age')"
                      class="border rounded-full px-2.5 py-1 text-xs font-medium whitespace-nowrap" :class="isDarkMode
                        ? 'border-transparent bg-neutral-800 text-gray-300'
                        : 'border-transparent bg-gray-100 text-gray-700'">
                      {{ getValue(contact, 'age') }}y
                    </span>
                    <span v-if="getValue(contact, 'sex')"
                      class="border rounded-full px-2.5 py-1 text-xs font-medium whitespace-nowrap" :class="isDarkMode
                        ? 'border-transparent bg-neutral-800 text-gray-300'
                        : 'border-transparent bg-gray-100 text-gray-700'">
                      {{ getValue(contact, 'sex') }}
                    </span>
                    <span v-if="getValue(contact, 'location')"
            ...
            <div class="flex-shrink-0">
              <i-mdi-check-circle v-if="isSelected(contact)" class="w-5 h-5"
        ...
        <!-- No Results -->
        <div v-else-if="shouldShowResults && !filteredContacts.length" class="text-center p-10 rounded-lg border"
          :class="isDarkMode
        ...
        <!-- Search Prompt -->
        <div v-else-if="!searchQuery.trim() && !showCreateForm" class="text-center p-10 rounded-lg border" :class="isDarkMode
          ? 'text-gray-400 bg-black border-transparent'
          : 'text-gray-500 bg-white border-transparent'">
        ...
        <div v-if="(selectedReporter || (extractedReporterId && extractedContactId)) && !showCreateForm"
          class="mt-5 p-4 border rounded-lg" :class="isDarkMode
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="font-semibold text-base mb-1" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
                {{ getValue(selectedReporter, 'fullname') || 'Reporter' }}
              </div>

              <div class="text-sm space-y-1 mb-3" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
              ...
              <div class="space-y-2">
                <div v-if="extractedReporterId" class="flex items-center gap-2 p-2 border rounded-md" :class="isDarkMode
                  ? 'bg-green-900/30 border-green-600/40'
                  : 'bg-green-50 border-green-300'">
                  <i-mdi-check-circle class="w-5 h-5 flex-shrink-0"
                    :class="isDarkMode ? 'text-green-400' : 'text-green-600'" />
                  <div class="flex-1">
                    <div class="text-xs font-medium" :class="isDarkMode ? 'text-green-400' : 'text-green-700'">
                      Reporter ID (Index 0)
                    </div>
                    <div class="text-sm font-bold" :class="isDarkMode ? 'text-green-300' : 'text-green-800'">
                      {{ extractedReporterId }}
                    </div>
                  </div>
                </div>

                <div v-if="extractedContactId" class="flex items-center gap-2 p-2 border rounded-md" :class="isDarkMode
                  ? 'bg-amber-900/30 border-amber-600/40'
                  : 'bg-amber-50 border-amber-300'">
                  <i-mdi-check-circle class="w-5 h-5 flex-shrink-0"
                    :class="isDarkMode ? 'text-amber-500' : 'text-amber-600'" />
                  <div class="flex-1">
                    <div class="text-xs font-medium" :class="isDarkMode ? 'text-amber-500' : 'text-amber-600'">
                      Contact ID (Index 5)
                    </div>
                    <div class="text-sm font-bold" :class="isDarkMode ? 'text-amber-300' : 'text-amber-800'">
                      {{ extractedContactId }}
                    </div>
                  </div>
                </div>
        ...
        <div v-if="showCreateForm" class="mt-5 p-5 border rounded-lg" :class="isDarkMode
          ...
          <div class="flex items-center justify-between mb-4">
            ...
            <button type="button" @click="closeCreateForm" class="transition-colors" :class="isDarkMode
          ...
          <div class="space-y-6">
            <!-- Basic Information -->
            <div class="border-b pb-4" :class="isDarkMode ? 'border-transparent' : 'border-transparent'">
              <h4 class="text-md font-semibold mb-4" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
                Basic Information
              </h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input v-model="reporterForm.fname" type="text"
                ...
                <div>
                  ...
                  <input v-model="reporterForm.dob" type="date" @change="handleDobChange"
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:border-transparent transition-all"
                    :class="isDarkMode
                      ? 'bg-neutral-800 border-transparent text-gray-100 focus:ring-amber-500'
                      : 'bg-gray-50 border-transparent text-gray-900 focus:ring-amber-600'" />
                  <p v-if="reporterForm.dob" class="text-xs mt-1"
                ...
                <div>
                  ...
                  <input v-model="reporterForm.age" type="number" :readonly="!!reporterForm.dob" :class="[
                ...
                <div>
                  <TaxonomySelect id="reporter-age-group" label="Age Group" v-model="reporterForm.ageGroup"
                    placeholder="Select Age Group (auto-filled from DOB)" root-key="AGE_GROUP"
                ...
                <div>
                  <TaxonomySelect id="reporter-sex" label="Sex" v-model="reporterForm.sex" placeholder="Select sex"
                    root-key="GENDER" />
                </div>

                <div>
                  <TaxonomySelect id="reporter-location" label="Location" v-model="reporterForm.location"
                    placeholder="Select location" root-key="LOCATION" :searchable="true" />
                </div>
              </div>
            </div>
            ...
            <div class="border-b pb-4" :class="isDarkMode ? 'border-transparent' : 'border-transparent'">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input v-model="reporterForm.phone" type="tel"
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:border-transparent transition-all"
                    :class="isDarkMode
                      ? 'bg-neutral-800 border-transparent text-gray-100 focus:ring-amber-500'
                      : 'bg-gray-50 border-transparent text-gray-900 focus:ring-amber-600'"
                ...
                <div>
                  ...
                  <input v-model="reporterForm.phone2" type="tel"
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:border-transparent transition-all"
                    :class="isDarkMode
                      ? 'bg-neutral-800 border-transparent text-gray-100 focus:ring-amber-500'
                      : 'bg-gray-50 border-transparent text-gray-900 focus:ring-amber-600'"
                ...
                <div class="md:col-span-2">
                  ...
                  <input v-model="reporterForm.email" type="email"
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:border-transparent transition-all"
                    :class="isDarkMode
                      ? 'bg-neutral-800 border-transparent text-gray-100 focus:ring-amber-500'
                      : 'bg-gray-50 border-transparent text-gray-900 focus:ring-amber-600'"
                ...
                <div class="md:col-span-2">
                  ...
                  <input v-model="reporterForm.landmark" type="text"
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:border-transparent transition-all"
                    :class="isDarkMode
                      ? 'bg-neutral-800 border-transparent text-gray-100 focus:ring-amber-500'
                      : 'bg-gray-50 border-transparent text-gray-900 focus:ring-amber-600'"
            ...
            <div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <TaxonomySelect id="reporter-nationality" label="Nationality" v-model="reporterForm.nationality"
                    placeholder="Select nationality" root-key="NATIONALITY" />
                </div>

                <div>
                  <TaxonomySelect id="reporter-language" label="Language" v-model="reporterForm.language"
                    placeholder="Select language" root-key="LANGUAGE" />
                </div>

                <div>
                  <TaxonomySelect id="reporter-tribe" label="Tribe" v-model="reporterForm.tribe"
                    placeholder="Select tribe" root-key="TRIBE" />
                </div>

                <div>
                  <TaxonomySelect id="reporter-id-type" label="ID Type" v-model="reporterForm.idType"
                ...
                <div>
                  ...
                  <input v-model="reporterForm.idNumber" type="text"
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:border-transparent transition-all"
                    :class="isDarkMode
                      ? 'bg-neutral-800 border-transparent text-gray-100 focus:ring-amber-500'
                      : 'bg-gray-50 border-transparent text-gray-900 focus:ring-amber-600'"
                ...
                <div>
                  ...
                  <div class="flex gap-4 mt-2">
                    <label class="flex items-center gap-1.5 cursor-pointer">
                      <input v-model="reporterForm.isRefugee" type="radio" value="1" class="w-4 h-4"
                    ...
                    <label class="flex items-center gap-1.5 cursor-pointer">
                      <input v-model="reporterForm.isRefugee" type="radio" value="0" class="w-4 h-4"
                        :class="isDarkMode ? 'text-amber-600' : 'text-amber-700'" />
                      <span class="text-sm" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
                        No
                      </span>
            ...
            <div class="flex gap-3 justify-end pt-4 border-t"
              ...
              <button type="button" @click="closeCreateForm" class="px-4 py-2 border rounded-lg transition-colors"
                :class="isDarkMode
                  ? 'bg-gray-700 text-gray-300 border-transparent hover:bg-gray-600'
                  : 'bg-white text-gray-700 border-transparent hover:bg-gray-50'">
                Cancel
              </button>
              <button type="button" @click="handleCreateReporter" :disabled="isCreating"
      ...
      <div class="flex gap-3 justify-between mt-6">
        <button type="button" class="px-4 py-2 bg-transparent border rounded-lg transition-colors" :class="isDarkMode
        ...
        <div class="flex gap-3">
          <button type="button"
            ...
            :class="isDarkMode
              ? 'bg-amber-600 hover:bg-amber-700'
              : 'bg-amber-700 hover:bg-amber-800'" :disabled="!extractedReporterId || !extractedContactId"
