<template>
  <div class="min-h-96">
    <div class="flex flex-col gap-3">
      ...
      <div class="border rounded-lg" :class="isDarkMode
        ? 'bg-black border-transparent'
        : 'bg-white border-transparent'">
        <div class="flex items-center justify-between p-3 px-4 border-b"
        ...
        <div class="p-4">
          <div v-if="formData.step1.selectedReporter || reporterId" class="p-4 border rounded-lg" :class="isDarkMode
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <!-- Reporter ID -->
                <div v-if="reporterId" class="flex items-center gap-2 p-2 border rounded-md" :class="isDarkMode
          ...
          <div v-else class="p-4 border rounded-lg text-center" :class="isDarkMode
      ...
      <div class="border rounded-lg" :class="isDarkMode
        ...
        <div class="p-3 px-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <div class="font-semibold text-sm" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
              GBV Related
            </div>
            <div :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
              {{ formData.step2.isGBVRelatedText || formData.step2.isGBVRelated || "N/A" }}
            </div>
          </div>
          <div>
            <div class="font-semibold text-sm" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
            ...
            <div :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
              {{ formData.step2.categoriesText || formData.step2.categories || "N/A" }}
            </div>
          </div>
          <div>
            <div class="font-semibold text-sm" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
            ...
            <div :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
              {{ formData.step2.incidentLocationText || formData.step2.incidentLocation || "N/A" }}
            </div>
          </div>
          ...
          <div>
            ...
            <div>
              <span class="inline-block px-2 py-1 rounded text-xs font-medium"
                :class="getPriorityClass(formData.step2.priority)">
          ...
          <div>
            ...
            <div>
              <span class="inline-block px-2 py-1 rounded text-xs font-medium"
                :class="getStatusClass(formData.step2.status)">
      ...
      <div class="border rounded-lg" :class="isDarkMode
        ...
        <div class="flex items-center justify-between p-3 px-4 border-b"
          ...
          <button class="flex items-center gap-2 px-3 py-1.5 border rounded-lg text-sm transition-colors" :class="isDarkMode
            ? 'bg-gray-700 text-gray-300 border-transparent hover:bg-gray-600'
            : 'bg-white text-gray-700 border-transparent hover:bg-gray-50'" @click="goToStep(3)">
            <i-mdi-pencil class="w-4 h-4" />
            Edit
          </button>
        </div>
        <div class="p-3 px-4 grid grid-cols-1 gap-4">
          <!-- Clients -->
          <div>
            <div class="font-semibold text-sm mb-2" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
              Clients ({{ formData.step3.clients.length }})
            </div>
            <div v-if="formData.step3.clients.length > 0" class="flex flex-col gap-2">
              <div v-for="(client, index) in formData.step3.clients" :key="index" class="p-2 rounded border" :class="isDarkMode
                ? 'bg-gray-700/50 border-transparent'
                : 'bg-gray-50 border-transparent'">
                <div class="font-medium text-sm" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
                  {{ client.name || 'Unnamed' }}
                </div>
            ...
            <div v-else class="text-sm italic" :class="isDarkMode ? 'text-gray-500' : 'text-gray-500'">
              No clients added
            </div>
          </div>
          ...
          <div>
            ...
            <div v-if="formData.step3.perpetrators.length > 0" class="flex flex-col gap-2">
              <div v-for="(perpetrator, index) in formData.step3.perpetrators" :key="index" class="p-2 rounded border"
                :class="isDarkMode
                  ? 'bg-gray-700/50 border-transparent'
                  : 'bg-gray-50 border-transparent'">
                <div class="font-medium text-sm" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
          ...
          <div>
            ...
            <div v-if="formData.step3.attachments.length > 0" class="flex flex-col gap-2">
              <div v-for="(attachment, index) in formData.step3.attachments" :key="index"
                class="p-2 rounded border text-sm" :class="isDarkMode
                  ? 'bg-gray-700/50 border-transparent'
                  : 'bg-gray-50 border-transparent'">
                <div :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
                  {{ attachment.name }}
    ...
    <div class="mt-6 p-6 border-2 rounded-xl animate-in fade-in slide-in-from-bottom-4 duration-500" :class="isDarkMode
      ...
      <TaxonomySelect id="case-disposition" v-model="formData.step4.disposition" placeholder="Select final outcome..."
        root-key="DISPOSITION" hint="This is required for reporting compliance." @change="handleDispositionChange" />
    </div>
...
<script setup>
  ...
  function submitCase() {
    if (!props.formData.step4.disposition) {
      toast.error('Mandatory Field Missing', {
        description: 'Please select a Case Disposition before creating the case.'
      })
      return
    }
  ...
  function handleDispositionChange({ id, text }) {
    // Update parent form data for disposition
    props.formData.step4.dispositionText = text
  }
