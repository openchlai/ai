  ...
  <div class="min-h-96">
    <form class="flex flex-col gap-3.5" @submit.prevent="handleFormSubmit">
      <div>
        ...
        <div class="mb-5">
          ...
          <div class="flex flex-col gap-3">
            <div v-if="localForm.clients && localForm.clients.length > 0" class="flex flex-col gap-2">
              <div v-for="(client, index) in localForm.clients" :key="`client-${index}`"
                ...
                <div>
                  ...
                  <div class="text-xs" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
                    {{ client.age ? client.age + ' years' : 'Age unknown' }} • {{ client.sex || 'Gender unknown' }} • {{
                ...
                <button type="button"
                  ...
                  @click="handleRemoveClient(index)" title="Remove client">
                  <i-mdi-close class="w-4 h-4" />
                </button>
              </div>
            </div>

            <div v-else class="text-center p-5 border border-dashed rounded-lg" :class="isDarkMode
            ...
            <button type="button"
              ...
              :class="isDarkMode
                ...
                : 'bg-gray-700 hover:bg-black'" @click="$emit('open-client-modal')">
        ...
        <div class="mb-5">
          ...
          <div class="flex flex-col gap-3">
            <div v-if="localForm.perpetrators && localForm.perpetrators.length" class="flex flex-col gap-2">
              <div v-for="(perpetrator, index) in localForm.perpetrators" :key="index"
                class="flex items-center justify-between p-3 border rounded-lg" :class="isDarkMode
                  ? 'bg-gray-700 border-transparent'
                  : 'bg-gray-50 border-transparent'">
                <div>
                  ...
                  <div class="text-xs" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
                    {{ perpetrator.age || 'Unknown age' }} • {{ perpetrator.sex || 'Unknown gender' }} • {{
                      perpetrator.location || 'Unknown location' }}
                  </div>
                </div>
                <button type="button"
                  class="w-6 h-6 flex items-center justify-center bg-red-600 text-white rounded-full hover:bg-red-700 text-sm transition-all hover:scale-110"
            ...
            <div v-else class="p-5 text-center italic border border-dashed rounded-lg" :class="isDarkMode
              ? 'text-gray-500 bg-black border-transparent'
              : 'text-gray-500 bg-white border-transparent'">
        ...
        <!-- Services Offered -->
        <div class="mb-5">
          <TaxonomyOptions id="services-offered" label="Services Offered" v-model="localForm.servicesOffered"
            placeholder="Select services..." root-key="SERVICE_OFFERED" @selection-change="handleServicesChange" />

          <div v-if="showReferralsField" class="mt-4 p-4 border rounded-lg animate-fadeIn" :class="isDarkMode
            ? 'bg-black border-transparent'
            : 'bg-gray-50 border-transparent'">
            <TaxonomyOptions id="referrals-type" label="Referral Types" v-model="localForm.referralsType"
              placeholder="Select referral types..." root-key="REFERRAL_TYPE" @selection-change="updateForm" />
          </div>

          <div v-if="showPoliceField" class="mt-4 p-4 border rounded-lg animate-fadeIn" :class="isDarkMode
            ? 'bg-black border-transparent'
            : 'bg-gray-50 border-transparent'">
            <label for="police-details" class="block font-semibold mb-2"
          ...
          <div v-if="showOthersField" class="mt-4 p-4 border rounded-lg animate-fadeIn" :class="isDarkMode
            ? 'bg-black border-transparent'
            : 'bg-gray-50 border-transparent'">
            <label for="other-services" class="block font-semibold mb-2"
              :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
        ...
        <div class="mb-5">
          <TaxonomySelect id="know about 116" label="How did you know about 116?" v-model="localForm.referralSource"
            placeholder="Select an option" root-key="KNOW_ABOUT_116" @change="updateForm" />
        </div>

        <!-- Attachments -->
        <div class="mb-5">
          <AttachmentUpload v-model="localForm.attachments" label="Case Attachments"
            description="Upload relevant documents, images, or files related to this case." :max-size-mb="10"
            @upload-complete="handleAttachmentUploadComplete" @upload-error="handleAttachmentUploadError" />
        </div>
      </div>
      ...
      <div class="flex gap-3 justify-between mt-6 pt-5 border-t"
        ...
        <button type="button" class="px-4 py-2 border rounded-lg transition-colors" :class="isDarkMode
          ...
          : 'bg-white text-gray-700 border-transparent hover:bg-gray-50'" @click="goToStep(2)">
          Back
        </button>
...
<script setup>
  ...
  const showReferralsField = computed(() => {
    return localForm.servicesOffered.some(id =>
      String(id) === "362145" || // Referral to Medical
      String(id) === "362146" || // Referral to Legal
      String(id) === "362147"    // Referral to Psychosocial
      // These IDs should ideally be in TAXONOMY_ROOTS or WORKFLOW_TRIGGERS
    ) || selectedServicesOptions.value.some(option => option.text?.toLowerCase().includes('referral'))
  })

  const showPoliceField = computed(() => {
    return localForm.servicesOffered.some(id => String(id) === String(taxonomyStore.triggers.SERVICE_POLICE)) ||
      selectedServicesOptions.value.some(option => option.text?.toLowerCase().includes('police'))
  })

  const showOthersField = computed(() => {
    return localForm.servicesOffered.some(id => String(id) === String(taxonomyStore.triggers.SERVICE_OTHER)) ||
      selectedServicesOptions.value.some(option => option.text?.toLowerCase().includes('other'))
  })
