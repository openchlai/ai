<template>
  <LegacyCaseLayout>
    ...
    <template #sidebar>
      ...
      >
        <div class="flex justify-between items-center">
          ...
          <button 
            v-if="formData.step1.selectedReporter"
            @click="clearReporter"
        ...
        <div v-if="!formData.step1.selectedReporter">
           <div class="relative">
             <input 
               v-model="formData.step1.searchQuery"
               type="text" 
           ...
           <div 
             v-if="formData.step1.filteredContacts.length > 0"
             class="mt-2 max-h-60 overflow-y-auto border rounded-lg divide-y"
             :class="isDarkMode ? 'bg-neutral-800 border-neutral-700 divide-neutral-700' : 'bg-white border-gray-200 divide-gray-100'"
           >
             <div 
               v-for="contact in formData.step1.filteredContacts" 
               :key="contact._id"
               class="p-2 cursor-pointer hover:bg-opacity-50 transition-colors"
               :class="isDarkMode ? 'hover:bg-neutral-700' : 'hover:bg-gray-50'"
               @click="selectReporter(contact)"
             >
               <div class="text-sm font-medium" :class="isDarkMode ? 'text-gray-200' : 'text-gray-900'">{{ getReporterName(contact) }}</div>
        ...
        <div v-else class="space-y-3">
          ...
          <div class="flex flex-wrap gap-2 text-xs">
            <span class="px-2 py-1 rounded bg-gray-100 dark:bg-neutral-800 text-gray-600 dark:text-gray-400">
          ...
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" v-model="formData.step1.reporterIsClient" class="w-4 h-4 rounded text-amber-600 focus:ring-amber-500 border-gray-300">
            <span class="text-sm" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">Reporter is also a Client</span>
          </label>
        </div>
      </div>
      ...
      >
        <div class="flex justify-between items-center">
          ...
          <button
            type="button"
            class="w-6 h-6 flex items-center justify-center rounded-full bg-gray-100 dark:bg-neutral-800 hover:bg-gray-200 dark:hover:bg-neutral-700 transition-colors"
        ...
        <div class="space-y-2">
          <div 
            v-for="(client, idx) in formData.step3.clients" 
            :key="idx" 
            class="p-2 rounded border flex justify-between items-center"
            :class="isDarkMode ? 'bg-neutral-800 border-neutral-700' : 'bg-gray-50 border-gray-200'"
          >
        ...
        <button 
           type="button"
           class="w-full py-1.5 text-sm border border-dashed rounded text-gray-500 hover:text-gray-700 hover:border-gray-400 dark:border-neutral-700 dark:hover:border-neutral-600 transition-colors"
           @click="openClientModal"
        >
          + Add a Client
        </button>
      ...
      >
         ...
         <div v-if="formData.step3.perpetrators.length === 0" class="text-xs text-gray-500 italic">No perpetrators added.</div>

         <div class="space-y-2">
          <div 
            v-for="(perp, idx) in formData.step3.perpetrators" 
            :key="idx" 
            class="p-2 rounded border flex justify-between items-center"
            :class="isDarkMode ? 'bg-neutral-800 border-neutral-700' : 'bg-gray-50 border-gray-200'"
          >
      ...
      >
        ...
        <AttachmentUpload
            v-model="formData.step3.attachments"
            label=""
            description=""
            compact
            :max-size-mb="10"
      ...
      >
        <BaseOptions
            ...
            v-model="formData.step3.servicesOffered"
         ...
         <BaseSelect
            ...
            v-model="formData.step3.referralSource"
    ...
    <template #main>
      <div class="space-y-6">
        ...
        <!-- Department (Required) -->
        <div>
          <label class="block font-semibold mb-2" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">Department *</label>
           <div class="flex gap-4">
            <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="formData.step2.department" type="radio" value="116" class="w-4 h-4 text-amber-600 focus:ring-amber-500">
              <span :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">116</span>
            </label>
             <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="formData.step2.department" type="radio" value="labor" class="w-4 h-4 text-amber-600 focus:ring-amber-500">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
           <!-- Case Category -->
            ...
            <BaseSelect
              label="GBV Related? *"
              v-model="formData.step2.isGBVRelated"
              placeholder="Select option"
              :category-id="118"
            />
        </div>
        ...
        <BaseTextarea
            ...
            v-model="formData.step2.narrative"
         ...
         <!-- Plan -->
         <div>
            <label class="block font-semibold mb-2" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">Case Plan</label>
            <textarea 
               v-model="formData.step2.plan" 
    ...
    <template #lower>
       <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <BaseSelect
              ...
              label="State of Case in Justice System"
              v-model="formData.step2.justiceSystemState"
           ...
           <BaseSelect
              ...
              v-model="formData.step2.generalAssessment"
    ...
    <template #footer>
      <div class="flex flex-col md:flex-row gap-6 items-end">
         <div class="flex-1 w-full grid grid-cols-1 md:grid-cols-3 gap-6">
            <!-- Priority -->
            <div>
              <label class="block font-semibold mb-2" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">Priority *</label>
              <select 
                v-model="formData.step2.priority"
            ...
            <!-- Status -->
            <div>
              <label class="block font-semibold mb-2" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">Status *</label>
               <select 
                v-model="formData.step2.status"
                class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 outline-none"
                :class="isDarkMode ? 'bg-neutral-800 border-neutral-700 text-white' : 'bg-gray-50 border-gray-200 text-gray-900'"
              >
                <option value="">Select</option>
            ...
            <div>
               ...
               <label class="block font-semibold mb-2" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">Escalated To</label>
               <select 
                 v-model="formData.step2.escalatedTo"
                 class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 outline-none"
                 :class="isDarkMode ? 'bg-neutral-800 border-neutral-700 text-white' : 'bg-gray-50 border-gray-200 text-gray-900'"
               >
         ...
         <div class="flex gap-4 shrink-0">
            <button 
              ...
              :class="isDarkMode ? 'border-neutral-700 text-gray-300 hover:bg-neutral-800' : 'border-gray-300 text-gray-700 hover:bg-gray-50'"
              @click="cancel"
            >
              Cancel
            </button>
            <button 
              type="button" 
              class="px-6 py-2.5 rounded-lg font-medium text-white transition-colors shadow-lg"
  ...
  <!-- Modals -->
  <ClientModalLegacy
    :is-open="clientModalOpen"
    :client-data="clientForm"
    @close="closeClientModal"
    @update="handleLegacyClientUpdate"
  />

  <PerpetratorModalLegacy
    :is-open="perpetratorModalOpen"
    :perpetrator-data="perpetratorForm"
    @close="closePerpetratorModal"
    @create="handleLegacyPerpetratorCreate"
  />

  <ReporterModalLegacy
    :is-open="createReporterOpen"
    :reporter-data="reporterForm"
    @close="createReporterOpen = false"
