<template>
  <aside class="fixed left-0 top-0 h-screen w-64 border-r border-transparent flex flex-col shadow-xl" :class="isDarkMode
    ...
    <div class="flex-shrink-0 flex flex-col items-center justify-center p-2 pb-4">
      <img src="@/assets/images/Openchs logo.png" alt="OpenCHS Logo" class="h-48 w-auto object-contain" />
      <h2 class=" text-2xl font-bold tracking-wide" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
        OPENCHS
      </h2>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto px-6 py-2">
      <div class="flex flex-col gap-1.5" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">

        <!-- Dashboard - All roles have access -->
        <RouterLink v-if="hasPermission('dashboard')" to="/"
        ...
        <RouterLink v-if="hasPermission('calls')" to="/calls"
          class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200"
        ...
        <RouterLink v-if="hasPermission('messages')" to="/messages"
          class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200"
        ...
        <!-- AI Predictions - counsellor, supervisor, case manager, admin (same as messages permission) -->
        <RouterLink v-if="hasPermission('messages')" to="/ai-predictions"
          class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200"
          :class="getNavLinkClass($route.path === '/ai-predictions')">
          <i-mdi-robot-outline class="w-5 h-5" />
          <span class="text-sm">AI Predictions</span>
        </RouterLink>

        <!-- Activities - admin only -->
        <RouterLink v-if="hasPermission('activities')" to="/activities"
          class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200"
        ...
        <RouterLink v-if="hasPermission('qa')" to="/qa"
          class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200"
        ...
        <!-- Users - admin only -->
        <RouterLink v-if="hasPermission('users')" to="/users"
          class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200"
        ...
        <RouterLink v-if="hasPermission('reports')" to="/reports"
          class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200"
        ...
        <RouterLink v-if="hasPermission('wallboard')" to="/wallboard"
          class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200"
        ...
        <!-- FAQs - All main roles -->
        <RouterLink v-if="hasPermission('faqs')" to="/faqs"
          class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200"
        ...
        <div v-if="authStore.isPartner || authStore.isMediaAccount" class="px-4 py-6 text-center">
          <div class="text-xs mb-2" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">
            <i-mdi-information-outline class="w-4 h-4 inline" />
          </div>
          <p class="text-xs leading-relaxed" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">
            {{ authStore.isPartner ? 'Partner' : 'Media' }} accounts have limited access. Contact your administrator for
            more information.
          </p>
        </div>
    ...
    <div class="flex-shrink-0 p-6 pt-4 border-t border-transparent">
      <div class="flex items-center gap-3 px-4 py-3 rounded-xl mb-3" :class="isDarkMode ? 'bg-black' : 'bg-gray-50'">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium truncate" :class="isDarkMode ? 'text-gray-200' : 'text-gray-900'">
        ...
        <button class="p-2 rounded-lg transition-all duration-300 flex items-center justify-center flex-shrink-0"
          :class="isDarkMode
            ...
            : 'hover:bg-amber-600/10 text-amber-700'" @click="$emit('toggle-theme')"
