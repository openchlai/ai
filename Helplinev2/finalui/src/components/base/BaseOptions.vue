<template>
  <div class="relative w-full">
    <label v-if="label" :for="id" class="block text-sm font-semibold mb-1.5"
    ...
    <div :class="[
      ...
      isOpen
        ? isDarkMode
          ? 'border-[#2DD4BF] ring-2 ring-[#2DD4BF]/50'
          : 'border-[#003366] ring-2 ring-[#003366]/50'
        : isDarkMode
          ? 'border-transparent hover:border-[#2DD4BF]'
          : 'border-transparent hover:border-[#003366]',
      selectedOptions.length > 0 ? '' : ''
    ]">
      <!-- Selected items display -->
      <div v-if="selectedOptions.length > 0" class="flex flex-wrap gap-1.5 p-2 pb-1 border-b"
        :class="isDarkMode ? 'border-transparent' : 'border-transparent'">
        <div v-for="option in selectedOptions" :key="option.value"
          class="inline-flex items-center gap-1.5 px-2 py-1 rounded text-xs font-medium" :class="isDarkMode
          ...
          <button type="button"
            ...
            @click="removeSelection(option)" :title="`Remove ${option.text}`">
            ×
          </button>
        </div>
      </div>
      ...
      <div :class="[
        ...
        <span v-if="selectedOptions.length === 0" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">
          {{ placeholder || 'Select options...' }}
        </span>
        <span v-else class="font-medium" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
      ...
      <div v-if="isOpen"
        ...
        <div v-if="loading" class="flex items-center justify-center gap-2 p-5 text-sm"
        ...
        <div v-else-if="error" class="flex flex-col items-center justify-center gap-3 p-5 text-sm"
          :class="isDarkMode ? 'text-red-400' : 'text-red-600'">
          <span class="text-xl">⚠️</span>
          <span>{{ error }}</span>
          <button type="button"
            class="px-3 py-1.5 text-white border-none rounded text-xs cursor-pointer transition-colors" :class="isDarkMode
              ? 'bg-[#008080] hover:bg-[#006666]'
              : 'bg-[#003366] hover:bg-[#002244]'" @click="fetchOptions">
            Retry
          </button>
        </div>

        <div v-else-if="options.length === 0" class="flex flex-col items-center justify-center gap-2 p-5 text-sm"
        ...
        <div v-else class="flex flex-col">
          ...
          <div class="max-h-[200px] overflow-y-auto">
            <label v-for="option in options" :key="option.value" :class="[
              ...
              isSelected(option)
                ? isDarkMode
                  ? 'bg-[#008080]/10 border-l-4 border-l-[#2DD4BF]'
                  : 'bg-[#003366]/5 border-l-4 border-l-[#003366]'
                : isDarkMode
                  ? 'hover:bg-gray-700'
                  : 'hover:bg-gray-50'
            ]">
              <input type="checkbox" :value="option.value" :checked="isSelected(option)"
                @change="toggleSelection(option)" class="mt-0.5 cursor-pointer"
                :class="isDarkMode ? 'accent-[#2DD4BF]' : 'accent-[#003366]'">
              <div class="flex-1 min-w-0">
                <span class="text-sm font-medium block" :class="isDarkMode ? 'text-gray-100' : 'text-gray-900'">
                  {{ option.text }}
                </span>
                <span v-if="option.description" class="block text-xs mt-0.5"
                  :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
                  {{ option.description }}
                </span>
              </div>
