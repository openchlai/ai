...
>>>>>>> main
      ...
      <div class="flex gap-3">
        <button @click="$emit('cancel')" type="button" class="px-4 py-2 border rounded-lg transition-colors font-medium"
          :class="isDarkMode
    ...
    <div class="space-y-4">
      ...
      <AccordionSection title="Case Information" :isOpen="openSections.caseInfo" :badge="null"
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <textarea v-model="formData.narrative" rows="6"
          ...
          <div>
            ...
            <textarea v-model="formData.plan" rows="4"
              class="w-full px-3 py-2 border rounded-lg text-sm transition-all focus:outline-none focus:ring-2 focus:border-transparent resize-vertical"
              :class="isDarkMode
                ? 'bg-gray-700 border-transparent text-gray-100 placeholder-gray-500 focus:ring-amber-500'
                : 'bg-gray-50 border-transparent text-gray-900 placeholder-gray-400 focus:ring-amber-600'"
              placeholder="Outline planned interventions..."></textarea>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select v-model="formData.escalated_to_id"
                class="w-full px-3 py-2 border rounded-lg text-sm transition-all focus:outline-none focus:ring-2 focus:border-transparent"
                :class="isDarkMode
                  ? 'bg-gray-700 border-transparent text-gray-100 focus:ring-amber-500'
                  : 'bg-gray-50 border-transparent text-gray-900 focus:ring-amber-600'">
      ...
      <!-- 2. Clients Section -->
      <AccordionSection title="Clients" :isOpen="openSections.clients" :badge="formData.clients_case.length"
...
>>>>>>> main
              ...
              <div class="flex gap-2">
                <button type="button" @click="editClient(index)"
                  class="px-3 py-1.5 text-xs font-medium rounded transition-colors" :class="isDarkMode
                    ? 'bg-amber-600 text-white hover:bg-amber-700'
                    : 'bg-amber-700 text-white hover:bg-amber-800'">
                  Edit
                </button>
                <button type="button" @click="removeClient(index)"
...
>>>>>>> main
      ...
      <AccordionSection title="Perpetrators" :isOpen="openSections.perpetrators"
        :badge="formData.perpetrators_case.length" @toggle="toggleSection('perpetrators')">
        <div class="space-y-3">
          <!-- Perpetrator List -->
...
>>>>>>> main
              ...
              <div class="flex gap-2">
                <button type="button" @click="editPerpetrator(index)"
                  class="px-3 py-1.5 text-xs font-medium rounded transition-colors" :class="isDarkMode
                    ? 'bg-amber-600 text-white hover:bg-amber-700'
                    : 'bg-amber-700 text-white hover:bg-amber-800'">
                  Edit
                </button>
                <button type="button" @click="removePerpetrator(index)"
                  class="px-3 py-1.5 text-xs font-medium bg-red-600 text-white rounded hover:bg-red-700 transition-colors">
                  Remove
                </button>
              </div>
...
>>>>>>> main
          ...
          <button type="button" @click="openPerpetratorModal"
            class="w-full px-4 py-2 border-2 border-dashed rounded-lg transition-colors font-medium flex items-center justify-center gap-2"
            :class="isDarkMode
              ? 'border-transparent text-gray-400 hover:border-amber-500 hover:text-amber-500 hover:bg-amber-900/20'
              : 'border-transparent text-gray-600 hover:border-amber-600 hover:text-amber-700 hover:bg-amber-50'">
      ...
      <AccordionSection title="Attachments" :isOpen="openSections.attachments" :badge="formData.attachments_case.length"
        @toggle="toggleSection('attachments')">
        <AttachmentUpload v-model="formData.attachments_case" label="Case Attachments"
          description="Upload or manage case documents" @upload-complete="handleAttachmentUpload" />
      </AccordionSection>
    </div>
...
>>>>>>> main
  ...
  <!-- Modals -->
  <ClientModal v-if="clientModalOpen" :clients="[]" :clientForm="clientForm" :currentClientStep="currentClientStep"
    @close-modal="closeClientModal" @update-client-form="updateClientForm" @prev-client-step="prevClientStep"
    @next-client-step="nextClientStep" @add-client="addClient" />

  <PerpetratorModal v-if="perpetratorModalOpen" :perpetrators="[]" :perpetratorForm="perpetratorForm"
    :currentPerpetratorStep="currentPerpetratorStep" :perpetratorModalOpen="perpetratorModalOpen"
    @close-modal="closePerpetratorModal" @update-perpetrator-form="updatePerpetratorForm"
    @prev-perpetrator-step="prevPerpetratorStep" @next-perpetrator-step="nextPerpetratorStep"
...
<script setup>
  ...
  onMounted(() => {
    ...
    formData.reporter_uuid_id = getCaseValue('reporter_uuid_id') || getCaseValue('reporter_id') || ''
    formData.contact_uuid_id = getCaseValue('contact_uuid_id') || getCaseValue('reporter_contact_id') || ''
    formData.dept = String(getCaseValue('dept') || '0')
    formData.knowabout116_id = getCaseValue('knowabout116_id') || ''
  ...
  const handleReferralsChange = (selectionData) => {
    formData.referals = selectionData.values || []
  }
  ...
  // Save handler
  const handleSave = () => {
    console.log('📝 EditCaseForm handleSave called. Current data:', JSON.parse(JSON.stringify(formData)))

    // Validate required fields
    const errors = []
    if (!formData.narrative || !String(formData.narrative).trim()) errors.push('Narrative')
    if (!formData.case_category_id) errors.push('Case Category')

    if (errors.length > 0) {
      console.warn('❌ Validation failed for fields:', errors)
      toast.error('Required fields missing or invalid', {
        description: `Missing: ${errors.join(', ')}. Please check all required fields.`
      })
      return
    }

    console.log('🚀 Validation passed. Emitting submit with payload.')
