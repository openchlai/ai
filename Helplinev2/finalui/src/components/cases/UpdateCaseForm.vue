<template>
  <div class="space-y-6">
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      <!-- Case Plan (Mandatory) -->
      <FormField v-model="localForm.plan" label="Case Plan Update" type="textarea" :rows="4"
        placeholder="Enter case plan update..." required :error="errors.plan" />

      <!-- Justice System Status -->
      <div>
        <TaxonomySelect id="justice-system-update" label="State of the Case in the Justice System"
          v-model="localForm.justice_id" placeholder="Select justice system status" root-key="JUSTICE_SYSTEM_STATE"
          @change="handleJusticeChange" />
      </div>

      <!-- General Case Assessment -->
      <div>
        <TaxonomySelect id="assessment-update" label="General Case Assessment" v-model="localForm.assessment_id"
          placeholder="Select assessment status" root-key="GENERAL_ASSESSMENT" @change="handleAssessmentChange" />
      </div>

      <!-- Priority (Mandatory) -->
      <FormField v-model="localForm.priority" label="Priority" type="select" :options="priorityOptions" required
        :error="errors.priority" />

      <!-- Status (Mandatory) -->
      <FormField v-model="localForm.status" label="Status" type="select" :options="statusOptions" required
        :error="errors.status" />

      <!-- Escalated To -->
      <div>
        <label class="block text-sm font-semibold mb-2" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
          Escalated To
        </label>
        ...
        <!-- User Select -->
        <select v-else v-model="localForm.escalated_to_id"
          class="w-full px-4 py-3 rounded-lg border focus:ring-2 focus:ring-offset-0 transition-all" :class="isDarkMode
            ? 'bg-black border-transparent text-gray-300 focus:ring-amber-500'
            : 'bg-white border-transparent text-gray-900 focus:ring-amber-500'" @change="handleEscalationChange">
      ...
      <!-- Disposition -->
      <div>
        <TaxonomySelect id="disposition-update" label="Disposition" v-model="localForm.disposition_id"
