...
>>>>>>> main
        ...
        <div class="flex items-center gap-2 text-sm">
          ...
          <span v-else class="font-semibold" :class="isDarkMode ? 'text-gray-200' : 'text-gray-900'">
            Case #{{ getCaseValue('id') }}
          </span>

          <span v-if="currentView !== 'details'" :class="isDarkMode ? 'text-gray-600' : 'text-gray-400'">
            /
          </span>

          <span v-if="currentView === 'update'" class="font-semibold"
            :class="isDarkMode ? 'text-amber-500' : 'text-amber-700'">
            Update Case
          </span>
          <span v-if="currentView === 'edit'" class="font-semibold"
            :class="isDarkMode ? 'text-amber-500' : 'text-amber-700'">
            Edit Case
          </span>
          <span v-if="currentView === 'history'" class="font-semibold"
            :class="isDarkMode ? 'text-amber-500' : 'text-amber-700'">
            Case History
          </span>
        </div>
        ...
        <!-- Close button -->
        <button @click="handleClose" class="p-2 rounded-lg transition-colors" :class="isDarkMode
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        <div v-if="currentView === 'details'" class="p-6 space-y-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            <!-- Priority Card -->
            <div class="p-4 rounded-lg border" :class="getPriorityCardClass(getCaseValue('priority'))">
              <div class="text-xs font-semibold uppercase mb-1" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
                Priority
              </div>
            ...
            <!-- Status Card -->
            <div class="p-4 rounded-lg border" :class="getStatusCardClass(getCaseValue('status'))">
              <div class="text-xs font-semibold uppercase mb-1" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
                Status
              </div>
              <div class="text-2xl font-bold">
...
>>>>>>> main
          ...
          <div class="grid grid-cols-3 gap-3">
            <button @click="currentView = 'update'"
...
>>>>>>> main
        ...
        <!-- Update View -->
        <UpdateCaseForm v-if="currentView === 'update'" :caseItem="caseItem" :cases_k="cases_k"
          :isSubmitting="isSubmitting" @submit="handleUpdate" @cancel="currentView = 'details'" />

        <!-- Edit View -->
        <EditCaseForm v-if="currentView === 'edit'" :caseItem="caseItem" :cases_k="cases_k" :isSubmitting="isSubmitting"
          @submit="handleEdit" @cancel="currentView = 'details'" />

        <!-- History View -->
        <CaseHistoryView v-if="currentView === 'history'" :caseItem="caseItem" :cases_k="cases_k"
          :activities="case_activities" :activities_k="case_activities_k" @back="currentView = 'details'" />
      </div>
    </div>
    ...
    <div v-else class="h-full flex flex-col items-center justify-center gap-4 p-6">
      ...
      <p class="text-center font-medium" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
        Failed to load case details
      </p>
      <button @click="handleClose" class="px-4 py-2 rounded-lg transition-colors" :class="isDarkMode
...
<script setup>
  ...
  const props = defineProps({
    ...
    cases_k: Object,
    case_activities: Array,
    case_activities_k: Object,
    loading: Boolean,
  ...
  // Auto-switch back to details view when case is updated
  watch(() => props.caseItem, (newVal, oldVal) => {
    if (newVal && oldVal && (currentView.value === 'edit' || currentView.value === 'update')) {
      currentView.value = 'details'
    }
  })
