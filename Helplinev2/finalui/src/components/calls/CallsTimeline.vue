<template>
  <div class="space-y-6">
    <div v-for="(group, label) in groupedCalls" :key="label" class="shadow-lg rounded-xl border overflow-hidden p-0"
      :class="isDarkMode
        ? 'bg-black border-transparent'
        : 'bg-white border-transparent'">
      <h2 class="text-base font-semibold p-4 border-b uppercase tracking-wide" :class="isDarkMode
      ...
      <div>
        <div v-for="(call, index) in group" :key="index" @click="handleSelect(call)"
          ...
          <!-- Icon -->
          <div class="w-10 h-10 flex items-center justify-center rounded-xl flex-shrink-0" :class="isDarkMode
          ...
          <!-- Details -->
          <div class="flex-1">
            <div class="font-medium text-sm" :class="isDarkMode ? 'text-gray-200' : 'text-gray-900'">
            ...
            <div class="text-xs mt-1" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
              Agent:
              <span class="font-medium" :class="isDarkMode ? 'text-amber-500' : 'text-amber-700'">
          ...
          <!-- QA Button -->
          <button @click.stop="emitCreateQA(call)" :disabled="!isAnswered(call)" :class="[
    ...
    <div v-if="Object.keys(groupedCalls).length === 0" class="text-center py-12 rounded-lg border" :class="isDarkMode
...
<script setup>
  ...
  function isAnswered(call) {
    ...
    const status = (call[statusIndex] || '').toLowerCase()
