<template>
  <div class="relative border-l-2 pl-6 space-y-6" :class="isDarkMode ? 'border-transparent' : 'border-transparent'">

    <div v-for="user in store.users" :key="getValue(user, 'id')"
      class="relative shadow-xl rounded-lg p-6 border cursor-pointer transition-all duration-200" :class="isDarkMode
        ...
        : 'bg-white border-transparent hover:border-amber-600'" @click="selectUser(user)">
      <!-- Timeline Dot -->
      <div class="absolute -left-[1.875rem] top-6 w-6 h-6 rounded-full border-4" :class="isDarkMode
        ? 'bg-amber-600 border-transparent'
        : 'bg-amber-600 border-transparent'"></div>
      ...
      <p class="text-sm mt-1" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
        Username: <span class="font-medium" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
          {{ getValue(user, 'usn') }}
        </span>
      </p>

      <p class="text-sm mt-1" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
        Extension: <span class="font-medium" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
          {{ getValue(user, 'exten') || 'N/A' }}
        </span>
      </p>
      ...
      <!-- Role Badge -->
      <span class="inline-block mt-3 px-3 py-1 rounded-full text-xs font-medium uppercase border" :class="isDarkMode
    ...
    <Transition name="modal">
      <div v-if="selectedUser"
        ...
        <UserDetailsModal :user="selectedUser" :loading="loadingUser" @close="handleClose" @edit="handleEdit" />
      </div>
    </Transition>
    ...
    <Transition name="modal">
      <div v-if="loadingUser"
        class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div class="rounded-lg shadow-2xl p-8 border" :class="isDarkMode
