...
export const useNotificationsStore = defineStore('notificationsStore', {
  ...
  getters: {
    ...
    // Get formatted notifications with timestamps
    formattedNotifications: (state) => {
      const createdOnIndex = parseInt(state.notifications_k?.created_on?.[0] ?? -1)
      const readOnIndex = parseInt(state.notifications_k?.read_on?.[0] ?? -1)

      if (createdOnIndex === -1) return []

      return state.notifications.map(notification => {
        const timestamp = notification[createdOnIndex]
        const readTimestamp = readOnIndex !== -1 ? notification[readOnIndex] : null
