...
def mock_agent_notification_service():
    """Fixture to mock the agent_notification_service singleton"""
    with patch("app.api.call_session_routes.agent_notification_service") as mock_service:
...
@patch("app.api.call_session_routes.AGENT_SERVICE_AVAILABLE", new=False)
def test_get_agent_service_health_unavailable():
    ...
    assert "service not available" in response.json()["detail"]
