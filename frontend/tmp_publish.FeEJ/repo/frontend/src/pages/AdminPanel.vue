    ...
    <div class="sidebar" :class="{ collapsed: isSidebarCollapsed, 'mobile-open': mobileOpen }" id="sidebar">
      ...
      <div class="sidebar-content">
        ...
        <div class="nav-section">
          <a href="/dashboard" class="nav-item" :class="{ active: activeTab === 'dashboard' }"
          ...
          <a href="/cases" class="nav-item" :class="{ active: activeTab === 'cases' }"
            @click.prevent="setActiveTab('cases')">
            <div class="nav-icon">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
            ...
            <div class="nav-text">Case Managementx\</div>
          </a>

          <a href="/users" class="nav-item" :class="{ active: activeTab === 'users' }"
          ...
          <a href="/reports" class="nav-item" :class="{ active: activeTab === 'reports' }"
          ...
          <a href="/ai-assistant" class="nav-item" :class="{ active: activeTab === 'ai-assistant' }"
            @click.prevent="setActiveTab('ai-assistant')">
            <div class="nav-icon">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <a href="/categories" class="nav-item" :class="{ active: activeTab === 'categories' }"
            @click.prevent="setActiveTab('categories')">
            <div class="nav-icon">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <a href="/workflows" class="nav-item" :class="{ active: activeTab === 'workflows' }"
            @click.prevent="setActiveTab('workflows')">
            <div class="nav-icon">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
          ...
          <a href="/settings" class="nav-item" :class="{ active: activeTab === 'settings' }"
      ...
      <div class="header">
        ...
        <div class="header-actions">
          ...
          <button class="theme-toggle" @click="toggleTheme" id="theme-toggle">
            <svg v-if="currentTheme === 'dark'" id="moon-icon" width="20" height="20" viewBox="0 0 24 24" fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" stroke="currentColor" stroke-width="2"
                stroke-linecap="round" stroke-linejoin="round" />
            </svg>
            <svg v-else id="sun-icon" width="20" height="20" viewBox="0 0 24 24" fill="none"
              xmlns="http://www.w3.org/2000/svg">
        ...
        <div v-if="activeTab === 'dashboard'">
          <div class="dashboard-grid">
            <div class="dashboard-card">
              ...
              <div class="card-subtitle">+ **not yet** this month</div>
            </div>
            ...
            <div class="dashboard-card">
              ...
              <div class="card-subtitle">{{ criticalCases }} urgent cases</div>
            </div>
            ...
            <div class="dashboard-card">
              ...
              <div class="card-subtitle">{{ activeUsers }} online now</div>
            </div>
            ...
            <div class="dashboard-card">
              ...
              <div class="card-value">{{ resolvedPercentage }}%</div>
              <div class="card-subtitle">+not yet from last month</div>
            </div>
          </div>
          ...
          <div class="dashboard-content-grid">
            <div class="recent-cases-card">
              <div class="card-header">
                ...
                <button class="view-all-btn" @click="setActiveTab('cases')">
                  View All
                </button>
              </div>
              <div class="cases-list">
                <div v-for="(case_, index) in recentCases" :key="case_[cases_k.id?.[0]] || index" class="case-item"
                  @click="viewCase(case_[cases_k.id?.[0]])">
                  <div class="case-priority" :class="mapPriority(case_[cases_k.priority?.[0]])?.toLowerCase()"></div>
                  ...
                  <div class="case-info">
                    ...
                    <p class="case-date">
                      {{
                        cases_k.dt
                          ? new Date(
                            case_[cases_k.hr?.[0]] < 10000000000 ? case_[cases_k.hr?.[0]] * 1000 : case_[cases_k.hr?.[0]] *
                              3600 * 1000).toLocaleString() : 'No Date' }} </p>
                  </div>

                  <div class="case-status">
                    <span class="status-badge" :class="mapStatus(case_[cases_k.status?.[0]])?.toLowerCase()">
              ...
              <div class="chart-scroll">
                <svg :width="svgWidth" :height="svgHeight">
                  ...
                  <g v-for="tick in yTicks" :key="'grid-' + tick">
                    <line :x1="margin.left" :x2="svgWidth - margin.right" :y1="yScale(tick)" :y2="yScale(tick)"
                      stroke="#ddd" stroke-width="1" />
                  </g>

                  <!-- Bars -->
                  <g v-for="(bar, index) in chartData" :key="'bar-' + index">
                    <rect :x="margin.left + index * (barWidth + barSpacing)" :y="yScale(bar.rawValue)" :width="barWidth"
                      :height="svgHeight - margin.bottom - yScale(bar.rawValue)" fill="url(#barGradient)" />
                    <!-- X-axis labels (timestamps) -->
                    <text :x="margin.left + index * (barWidth + barSpacing) + barWidth / 2"
                      :y="svgHeight - margin.bottom + 15" text-anchor="middle" font-size="10">
                  ...
                  <g v-for="tick in yTicks" :key="'label-' + tick">
                    <text :x="margin.left - 5" :y="yScale(tick) + 3" text-anchor="end" font-size="10">
                      {{ tick }}
                    </text>
                  </g>

                  <!-- X-axis line -->
                  <line :x1="margin.left" :x2="svgWidth - margin.right" :y1="svgHeight - margin.bottom"
                    :y2="svgHeight - margin.bottom" stroke="#333" stroke-width="1.2" />

                  <!-- Y-axis line -->
                  <line :x1="margin.left" :x2="margin.left" :y1="margin.top" :y2="svgHeight - margin.bottom"
          ...
          <div class="table-card">
            ...
            <div class="table-container">
              <table class="cases-table">
                ...
                <tbody>
                  <tr v-for="(case_, index) in cases" :key="case_[cases_k.id?.[0]] || index" class="case-row">
                    <td class="case-cell">
                      <div class="case-info">
                    ...
                    <td>
                      <span class="priority-badge" :class="('low')">
                        {{ case_[cases_k.priority?.[0]] || 'Low' }}
                      </span>
                    </td>

                    <td>
                      <span class="status-badge" :class="('open')">
                    ...
                    <td>
                      <div class="case-actions">
                        <button class="action-btn view-btn" @click="viewCase(case_[cases_k.id])">
                        ...
                        <button class="action-btn edit-btn" @click="editCase(case_[cases_k.id])">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="table-card">
            ...
            <div class="table-container">
              <table class="users-table">
                ...
                <tbody>
                  <tr v-for="(user, index) in users" :key="user[users_k.id?.[0]] || index" class="user-row">
                    <td class="user-cell">
                      <div class="user-info">
                    ...
                    <td>
                      <div class="role-cell">
                        <select v-if="editingRole === user[users_k.id?.[0]]" v-model="user[users_k.role?.[0]]"
                          @change="saveUserRole(user[users_k.id?.[0]], user[users_k.role?.[0]])"
                          @blur="editingRole = null" class="role-select">
                          <option value="Admin">Admin</option>
                          <option value="Manager">Manager</option>
                          <option value="Case Worker">Case Worker</option>
                          <option value="Supervisor">Supervisor</option>
                        ...
                        <div v-else class="role-display" @click="startEditingRole(user[users_k.id?.[0]])">
                    ...
                    <td>
                      <span class="status-badge" :class="(user[users_k.is_active?.[0]] || '').toLowerCase()">
                    ...
                    <td>
                      <div class="user-actions">
                        <button class="action-btn edit-btn" @click="editUser(user[users_k.id?.[0]])">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" stroke="currentColor"
                              stroke-width="2" />
                        ...
                        <button class="action-btn deactivate-btn" @click="toggleUserStatus(user[users_k.id?.[0]])"
                          :class="{ 'activate-btn': user[users_k.is_active?.[0]] === 'Inactive' }">
                          <svg v-if="user[users_k.is_active?.[0]] === 'Active'" width="14" height="14"
          ...
          <div class="reports-grid">
            <div class="report-card">
              <div class="card-header">
                ...
                <button class="generate-btn" @click="generateReport('cases')">
                  Generate Report
                </button>
              </div>
                ...
                <div class="stats-grid">
                  ...
                  <div class="stat-item">
                    <div class="stat-value">
                      {{ reportStats.resolvedCases }}
                    </div>
                    <div class="stat-label">Resolved</div>
                  </div>
                  <div class="stat-item">
                    <div class="stat-value">
                      {{ reportStats.avgResolutionTime }}
                    </div>
                    <div class="stat-label">Avg Resolution Time</div>
                  </div>
                  <div class="stat-item">
                    <div class="stat-value">
                      {{ reportStats.satisfactionRate }}%
                    </div>
            ...
            <div class="report-card">
              <div class="card-header">
                ...
                <button class="generate-btn" @click="generateReport('team')">
                  Generate Report
                </button>
              </div>
              <div class="report-content">
          ...
          <div class="ai-assistant-container">
              ...
              <div class="chat-container">
                ...
                <div class="chat-input-container">
                  <input type="text" v-model="newMessage" @keyup.enter="sendMessage"
                    placeholder="Ask me anything about your cases, analytics, or organization..." class="chat-input" />
            ...
            <div class="ai-suggestions-card">
              ...
              <div class="suggestions-list">
                <button v-for="suggestion in aiSuggestions" :key="suggestion.id" class="suggestion-btn"
            ...
            <div class="workflows-list">
              <div v-for="workflow in workflows" :key="workflow.id" class="workflow-card">
                ...
                <div class="workflow-actions">
                  ...
                  <button class="action-btn toggle-btn" @click="toggleWorkflow(workflow.id)"
            ...
            <div class="modal-body">
              <div class="form-grid">
                ...
                <div class="form-group full-width">
                  ...
                  <textarea v-model="newCase.description" class="form-textarea" placeholder="Enter case description"
                    rows="4"></textarea>
                </div>
              </div>
            </div>
            <div class="modal-footer">
              <button class="cancel-btn" @click="showCreateCaseModal = false">
                Cancel
              </button>
              <button class="submit-btn" @click="createCase">
                Create Case
              </button>
            </div>
          </div>
            ...
            <div class="modal-body">
              <div class="form-grid">
                ...
                <div class="form-group full-width">
                  ...
                  <textarea v-model="newUser.welcomeMessage" class="form-textarea"
                    placeholder="Optional welcome message" rows="3"></textarea>
                </div>
              </div>
            </div>
            <div class="modal-footer">
              <button class="cancel-btn" @click="showInviteUserModal = false">
                Cancel
              </button>
              <button class="submit-btn" @click="inviteUser">
                Send Invitation
              </button>
            </div>
          </div>
        ...
        <div class="modal" v-if="showCreateCategoryModal">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="form-group">
                <label class="form-label">Description</label>
                <textarea v-model="newCategory.description" class="form-textarea" placeholder="Enter description"
                  rows="3"></textarea>
              </div>
              <div class="form-group">
                <label class="form-label">Color</label>
                <div class="color-picker">
                  <div v-for="color in categoryColors" :key="color" class="color-option"
                    :style="{ backgroundColor: color }" :class="{ selected: newCategory.color === color }"
                    @click="newCategory.color = color"></div>
                </div>
              </div>
            </div>
            <div class="modal-footer">
              <button class="cancel-btn" @click="showCreateCategoryModal = false">
                Cancel
              </button>
              <button class="submit-btn" @click="createCategory">
                Create Category
              </button>
            </div>
          </div>
        ...
        <div class="modal" v-if="showCreateWorkflowModal">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="form-group">
                <label class="form-label">Description</label>
                <textarea v-model="newWorkflow.description" class="form-textarea" placeholder="Enter description"
                  rows="3"></textarea>
              </div>
              <div class="form-group">
                ...
                <div class="workflow-steps-builder">
                  <div v-for="(step, index) in newWorkflow.steps" :key="index" class="step-builder">
                    ...
                    <button class="remove-step-btn" @click="removeWorkflowStep(index)"
            ...
            <div class="modal-footer">
              <button class="cancel-btn" @click="showCreateWorkflowModal = false">
                Cancel
              </button>
              <button class="submit-btn" @click="createWorkflow">
                Create Workflow
              </button>
            </div>
          </div>
          ...
          <div class="notifications-list">
            <div v-for="notification in notifications" :key="notification.id" class="notification-item"
              ...
              <div class="notification-icon" :class="notification.type">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path v-if="notification.type === 'case'"
                    d="M14 2H6C5.46957 2 4.96086 2.21071 4.58579 2.58579C4.21071 2.96086 4 3.46957 4 4V20C4 20.5304 4.21071 21.0391 4.58579 21.4142C4.96086 21.7893 5.46957 22 6 22H18C18.5304 22 19.0391 21.7893 19.4142 21.4142C19.7893 21.0391 20 20.5304 20 20V8L14 2Z"
                    stroke="currentColor" stroke-width="2" />
                  <path v-else-if="notification.type === 'user'" d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"
                    stroke="currentColor" stroke-width="2" />
                  <circle v-else-if="notification.type === 'user'" cx="9" cy="7" r="4" stroke="currentColor"
                    stroke-width="2" />
...
<script setup>
  ...
  // Local storage for different datasets
  ...
  const criticalCases = ref(0);
  const resolvedPercentage = ref(0);
  ...
  function calculateTotal() {
    totalCases.value = casesByStatus.value.reduce((sum, row) => {
      return sum + Number(row?.[1] || 0);
    }, 0);
  }

  function calculateActiveCases() {
    const active = casesByStatus.value.find((row) => row?.[0] === "1"); // assuming '1' = Active
    activeCases.value = Number(active?.[1]) || 0;
  }

  function calculateCriticalCases() {
    const critical = casesByPriority.value.find((row) => row?.[0] === "3"); // '3' = Critical
    criticalCases.value = Number(critical?.[1]) || 0;
  }

  function calculateUsers() {
    const rows = Array.isArray(userStore.users) ? userStore.users : [];
    totalUsers.value = rows.length;

    // index for "is_active" taken from users_k metadata (string index -> number)
    const isActiveIdxRaw = userStore.users_k?.is_active?.[0];
    const isActiveIdx = Number(isActiveIdxRaw);

    if (Number.isFinite(isActiveIdx)) {
      let count = 0;
      for (const row of rows) {
        if (Array.isArray(row) && Number(row[isActiveIdx]) === 1) {
          count += 1;
        }
      }
      ...
      activeUsers.value = 0;
      // optional: console.warn("users_k.is_active index not found");
    }
  }

  function calculateResolvedPercentage() {
    // Assuming the second element in casesByStatus (index 1) represents "Resolved"
    if (casesByStatus.value.length > 1) {
      const resolvedCount = Number(casesByStatus.value[1]?.[1]) || 0;
      console.log("Resolved count:", resolvedCount);
      resolvedPercentage.value =
        totalCases.value > 0
          ? ((resolvedCount / totalCases.value) * 100).toFixed(2)
          : 0;
    } else {
      resolvedPercentage.value = 0;
    }
  }

  function mapPriority(priorityCode) {
    switch (String(priorityCode)) {
      case "1":
        return "Low";
      case "2":
        return "Medium";
  ...
  const filteredCases = computed(() => {
    return cases.value.filter((case_) => {
      if (caseFilters.value.status && case_.status !== caseFilters.value.status)
        return false;
      if (
        caseFilters.value.priority &&
        case_.priority !== caseFilters.value.priority
      )
        return false;
      if (
        caseFilters.value.assignedTo &&
        case_.assignedTo !== caseFilters.value.assignedTo
      )
        return false;
      if (caseFilters.value.search) {
        ...
        if (
          ...
          !case_.clientName.toLowerCase().includes(search)
        )
          return false;
      }
      return true;
  ...
  const filteredUsers = computed(() => {
    return teamMembers.value.filter((user) => {
      if (userFilters.value.role && user.role !== userFilters.value.role)
        return false;
  ...
  const createCase = () => {
    if (
      !newCase.value.title ||
      !newCase.value.clientName ||
      !newCase.value.category
    ) {
      alert("Please fill in all required fields.");
      return;
    }

    const caseNumber = `CASE-${new Date().getFullYear()}-${String(
      cases.value.length + 1
    ).padStart(3, "0")}`;
    const newCaseObj = {
  ...
  const createWorkflow = () => {
    if (
      ...
      newWorkflow.value.steps.some((step) => !step.name)
    ) {
      alert("Please fill in all required fields.");
      return;
    }
  ...
  const toggleWorkflow = (workflowId) => {
    ...
    if (workflowIndex !== -1) {
      ...
      alert(
        `Workflow ${workflow.name} has been ${workflow.status.toLowerCase()}.`
      );
    }
  };
