  ...
  <div class="app-container">
    ...
    <div class="main-content">
      <div
        ...
        :class="{ 'chat-panel-open': showChatPanel }"
      >
        <div class="header">
          ...
          <div
            ...
            :class="[
              ...
              'fine-border',
              {
                selected:
                  selectedMessageId ===
                  message[messagesStore.pmessages_k.id[0]],
              },
            ]"
              ...
              <div class="call-time">
                {{
                  message[messagesStore.pmessages_k.dth[0]]
                    ? new Date(
                        message[messagesStore.pmessages_k.dth[0]] * 1000
                      ).toLocaleString()
                    : "N/A"
                }}
              </div>
    ...
    <div class="view-container" v-show="activeView === 'table'">
      <div
        ...
        class="calls-table-container"
      >
        <table class="calls-table">
          <thead>
            <tr>
              <th>
                {{ messagesStore.pmessages_k?.contacts?.[3] || "Contact" }}
              </th>
              <th>{{ messagesStore.pmessages_k?.src?.[3] || "Platform" }}</th>
              <th>
                {{ messagesStore.pmessages_k?.src_msg?.[3] || "Last Message" }}
              </th>
              <th>{{ messagesStore.pmessages_k?.dth?.[3] || "Time" }}</th>
              <th>
                {{ messagesStore.pmessages_k?.src_status?.[3] || "Status" }}
              </th>
            </tr>
          </thead>
          <tbody>
      ...
      <div v-else class="no-chats">No chats to display.</div>
    </div>
        ...
        <div class="chat-details-avatar">
          <div
            ...
            :style="{
              background: getAvatarColor(
                selectedMessage?.[messagesStore.pmessages_k.created_by[0]] || ''
              ),
            }"
          >
            {{
              selectedMessage?.[
                messagesStore.pmessages_k.created_by[0]
              ]?.charAt(0) || "?"
            }}
          </div>
          <<<<<<< HEAD
          <div class="chat-details-title">
            <span class="contact-name">
              {{
                selectedMessage?.[messagesStore.pmessages_k.contacts[0]] ||
                "Chat Details"
              }}
            </span>
            ...
            <span class="chat-status">
              {{
                selectedMessage?.[messagesStore.pmessages_k.src_status[0]] ||
                "Active"
              }}
            </span>
          </div>
      ...
      <div class="chat-details-actions">
        ...
        >
          ...
          >
            ...
            <path d="M14 11a5 5 0 0 0-7-7l-3 3a5 5 0 0 0 7 7l1-1" />
          </svg>
        </button>
        <button
          class="chat-action-btn create"
          @click="createCase"
          title="Create Case"
        >
          <svg
        ...
        <button
          class="chat-action-btn reporter"
          @click="editReporter"
      ...
      <div class="chat-details-content">
        <div class="chat-thread whatsapp-style">
          <div
            ...
            :class="[
              'chat-bubble',
              msg.sender === 'counsellor' ? 'sent' : 'received',
            ]"
          >
...
const getPlatformUnreadCount = (platformId) => {
  const platformKey = messagesStore.pmessages_k.find(
    (k) => k[3] === "platform"
  )?.[0];
  const statusKey = messagesStore.pmessages_k.find(
    (k) => k[3] === "status"
  )?.[0];
  return messagesStore.pmessages.filter(
    (msg) =>
...
const queueStatus = computed(() =>
  isInQueue.value ? "In Queue" : "Not in queue"
);
...
function archiveChat() {
  alert("Archive Chat (dummy)");
}
function editReporter() {
  alert("Add/Edit Reporter (dummy)");
}
...
const simulatedChatThread = computed(() => [
  {
    id: 1,
    sender: "user",
    text: selectedMessage?.text || "Hi, I need help with my case.",
    time: selectedMessage?.time || "09:00AM",
  },
  {
    id: 2,
    sender: "counsellor",
  ...
  },
  {
    id: 3,
    sender: "user",
    text: "I want to know the status of my report.",
    time: "09:02AM",
  },
  {
    id: 4,
    sender: "counsellor",
    text: "Your report is being processed. We will update you soon.",
    time: "09:03AM",
  },
...
.chat-action-btn.create {
  background: #4caf50;
  color: #fff;
}
.chat-action-btn.reporter {
  background: #757575;
  color: #fff;
}
...
/* Empty state */
.call-list:empty::before,
.calls-table tbody:empty::before {
...
.chat-modal-panel {
  ...
  flex-direction: column;
  overflow: hidden;
  animation: fadeInScale 0.25s cubic-bezier(0.4, 1.4, 0.6, 1) 1;
}
